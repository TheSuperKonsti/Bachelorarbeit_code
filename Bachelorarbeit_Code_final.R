...
years_region5 <- c(2002, 2007, 2012, 2017, 2022)
data_subset5 <- data_with_region %>% filter(!is.na(Region) & Year %in% years_region5)
region_names5 <- unique(data_subset5$Region)

results_by_region5 <- data.frame()
for (region in region_names5) {
  pop_vals <- data_subset5 %>% filter(Region == region) %>% pull(Population) %>% as.numeric()
  if (length(pop_vals) < 20) next
  d1 <- extract_digits(pop_vals, 1)
  d1 <- d1[d1 %in% as.character(1:9)]
  freq_1 <- table(factor(d1, levels = as.character(1:9)))
  rel_1 <- freq_1 / sum(freq_1)
  ks_1 <- max(abs(cumsum(rel_1) - cumsum(benford_probs_1digit)))
  mad_1 <- mean(abs(as.numeric(rel_1) - benford_probs_1digit))
  mad_1_class <- ifelse(mad_1 < 0.006, "Close",
                        ifelse(mad_1 < 0.012, "Acceptable",
                               ifelse(mad_1 < 0.015, "Marginal", "Nonconformity")))
  chi_1 <- suppressWarnings(chisq.test(freq_1, p = benford_probs_1digit, rescale.p = TRUE))
  ...
  d2 <- d2[d2 %in% as.character(10:99)]
  freq_2 <- table(factor(d2, levels = as.character(10:99)))
  rel_2 <- freq_2 / sum(freq_2)
  ks_2 <- max(abs(cumsum(rel_2) - cumsum(benford_probs_2digits)))
  mad_2 <- mean(abs(as.numeric(rel_2) - benford_probs_2digits))
  mad_2_class <- ifelse(mad_2 < 0.006, "Close",
                        ifelse(mad_2 < 0.012, "Acceptable",
                               ifelse(mad_2 < 0.015, "Marginal", "Nonconformity")))
  chi_2 <- suppressWarnings(chisq.test(freq_2, p = benford_probs_2digits, rescale.p = TRUE))
  
  results_by_region5 <- rbind(results_by_region5, data.frame(
    Region = region,
    Observations = length(pop_vals),
    KS_1st = round(ks_1, 4),
    MAD_1st = round(mad_1, 4),
...
colnames(results_by_region5) <- c(
  "Region", "Observations",
  "KS_1st", "MAD_1st", "MAD_1st_Class", "Chi2_1st_p",
  "KS_1to2", "MAD_1to2", "MAD_1to2_Class", "Chi2_1to2_p"
)

xtable_region5 <- xtable(
  results_by_region5,
  caption = "Aggregated Benford results by continent (5 benchmark years: 2002, 2007, 2012, 2017, 2022).",
  label = "tab:region5",
  digits = 4
)
print(xtable_region5, include.rownames = FALSE)

ks_threshold_year5 <- data.frame(
  Group = "Global (per benchmark year)",
  Observations = 193,
  KS_Threshold = round(1.36 / sqrt(193), 4)
)
ks_threshold_regions5 <- results_by_region5[, c("Region", "Observations")]
ks_threshold_regions5$KS_Threshold <- round(1.36 / sqrt(ks_threshold_regions5$Observations), 4)
colnames(ks_threshold_regions5) <- c("Group", "Observations", "KS_Threshold")
ks_threshold_all5 <- rbind(ks_threshold_year5, ks_threshold_regions5)

xtable_ks5 <- xtable(
  ks_threshold_all5,
  caption = "KS critical values by region and globally (5 benchmark years).",
  label = "tab:ks_thresholds_5",
  digits = c(0, 0, 0, 4)
)
print(xtable_ks5, include.rownames = FALSE)
